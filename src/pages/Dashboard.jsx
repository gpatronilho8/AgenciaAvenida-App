...
function ClickableStatCard({ title, value, icon: Icon, color = 'blue', subtitle, to }) {
...
export default function Dashboard() {
  ...
  return (
    <div>
      <PageHeader
        title={condominioAtual ? condominioAtual.nome : 'Dashboard Global'}
        subtitle={`Resumo — ${format(hoje, "MMMM 'de' yyyy", { locale: pt })}`}
      />

      {/* Stats row 1 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-5">
        <ClickableStatCard title="Recebimentos do Mês" value={`€${recebimentosMes.toFixed(2)}`} icon={Euro} color="green" subtitle="Quotas pagas este mês" to="/quotas" />
        <ClickableStatCard title="Saldo Total" value={`€${saldoTotal.toFixed(2)}`} icon={TrendingUp} color="blue" subtitle="Banco + Caixa" to="/movimentos" />
        <ClickableStatCard title="Ocorrências Pendentes" value={ocorrenciasPendentes} icon={AlertTriangle} color="orange" subtitle="Abertas ou em progresso" to="/ocorrencias" />
        <ClickableStatCard title="Quotas em Dívida" value={quotasVencidas} icon={CreditCard} color="red" subtitle="Vencidas sem pagamento" to="/quotas" />
      </div>

      {/* Stats row 2 */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        <ClickableStatCard title="Condomínios Ativos" value={condFilt.filter(c => c.ativo !== false).length} icon={Building2} color="blue" to="/condominios" />
        <ClickableStatCard title="Quotas Pendentes" value={quotasPendentes} icon={Clock} color="orange" to="/quotas" />
        <ClickableStatCard title="Despesas do Mês" value={`€${despesasMes.toFixed(2)}`} icon={CheckCircle} color="purple" to="/movimentos" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-xl border border-border shadow-sm">
          ...
          <div className="divide-y divide-border">
            {recentOcorrencias.length === 0 && (
              <p className="text-muted-foreground text-sm text-center py-8">Sem ocorrências pendentes</p>
            )}
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm">
          <div className="px-5 py-4 border-b border-border">
            <h3 className="font-semibold text-foreground">Quotas Vencidas</h3>
          </div>
          <div className="divide-y divide-border">
            {recentQuotas.length === 0 && (
              <p className="text-muted-foreground text-sm text-center py-8">Sem quotas vencidas</p>
            )}
            {recentQuotas.map(q => (
              <div key={q.id} className="px-5 py-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-foreground">{q.descricao || 'Quota'}</p>
                  <p className="text-xs text-muted-foreground">Venc: {q.data_vencimento}</p>
                </div>
                <span className="text-sm font-bold text-foreground">€{q.valor?.toFixed(2)}</span>
              </div>
            ))}
          </div>
        </div>
